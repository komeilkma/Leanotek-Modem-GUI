...
class LeanoIcons {
	...
	replace() {
		if ("undefined" == typeof document) throw new Error("`LeanoIcons.replace()` only works in a browser environment.");
		const e = document.querySelectorAll("[data-leano-icon]");
	...
	_replaceElement(e = {}) {
		...
		if (delete n.icon, this.icons[t]) {
			const r = this._toSvg(this.icons[t], {
					...n,
					class: `leano-icons leano-icons-${t} ${n.class}`
				}),
				o = (new DOMParser).parseFromString(r, "image/svg+xml").querySelector("svg");
			e.parentNode.replaceChild(o, e)
		} else console.warn(`[LeanoIcons] ${t} icon is not defined.`)
	}
	_getAttrs(e) {
		return Array.from(e.attributes).reduce(((e, n) => {
			const t = n.name.replace("data-leano-", "");
			return "size" === t ? (e.width = n.value, e.height = n.value) : e[t] = n.value, e
			return "stroke" === t ? (e.stroke = n.value) : e[t] = n.value, e
			return "fill" === t ? (e.fill = n.value) : e[t] = n.value, e
		}), {})
	}
	_toSvg(e, n = {}) {
		const t = {
